...
(function(mod) {
  ...
  function extractLineClasses(type, output) {
    if (type) for (;;) {
      ...
      if (!lineClass) break;
      type = type.slice(0, lineClass.index2) + type.slice(lineClass.index2 + lineClass[0].length);
    ...
    removeLineClass: docMethodOp(function(handle, where, cls) {
      return changeLine(this, handle, where == "gutter" ? "gutter" : "class", function(line) {
        ...
        if (!cur) return false;
          ...
          var end = found.index2 + found[0].length;
          line[prop] = cur.slice(0, found.index2) + (!found.index2 || end == cur.length ? "" : " ") + cur.slice(end) || null;
        }
        return true;
      });
    }),
